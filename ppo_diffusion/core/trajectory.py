...
class DiffusionSampler:
    ...
    def sample_with_trajectory_recording(
        ...
        for i, t in enumerate(timesteps):
            ...
            # Calculate log probability using same method as LoRA for consistency
            alpha_prod_t = self.scheduler.alphas_cumprod[t]
            sigma = ((1 - alpha_prod_t) ** 0.5)
            noise_magnitude = torch.mean(noise_pred_guided ** 2)
            
            # Gaussian log probability (simplified)
            step_log_prob = -0.5 * noise_magnitude / (sigma ** 2 + 1e-8)
            step_log_prob = step_log_prob * 0.1  # Scale for numerical stability

            # Check for NaN and replace with safe value
            if torch.isnan(step_log_prob) or torch.isinf(step_log_prob):
                step_log_prob = torch.tensor(-0.1, device=noise_pred_guided.device, requires_grad=True)
                print(f"⚠️ NaN detected in step_log_prob at timestep {t}, using safe fallback")
            ...
            del latent_model_input, scheduler_output, step_log_prob, noise_magnitude, sigma
